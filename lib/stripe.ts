...
export const createCheckoutSession = async ({ email }: { email: string }) => {
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `https://sandbox-23.vercel.app/dashboard/new-product`,
      cancel_url: `https://sandbox-23.vercel.app/`,
    });
  ...
  try {
   ...
   if (!customer || !customer.id) {
    ...
    const portal = await stripe.billingPortal.sessions.create({
      ...
      return_url: `https://sandbox-23.vercel.app/dashboard/my-products`,
    });
